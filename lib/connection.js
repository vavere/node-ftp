    ...
    function reentry(err, text, code) {
      ...
      if (!cmd) {
        ...
        if (!err) {
          ...
          self._mlsd = self._feat.indexOf('MLSD')>=0 // ????
...
FTP.prototype.list = function(path, zcomp, cb) {
  return this._list(this._mlsd ? 'MLSD' : 'LIST', path, zcomp, cb);
}

FTP.prototype._list = function(method, path, zcomp, cb) {
  var self = this, cmd;
    ...
    function final() {
      if (done && replies === 2) {
        ...
        for (var i = 0, len = entries.length; i < len; ++i) {
          var parsedVal = (method === 'LIST' ? Parser.parseListEntry(entries[i])
                                            : Parser.parseMLSDEntry(entries[i]));
          if (parsedVal !== null)
