...
Parser.parseMLSDEntry = function(line) {
  var kvs = line.split(RE_MLSD_SEP);
  var ret = {
    name: kvs.pop().substring(1)
  };
  kvs.forEach(function(kv) {
    kv = kv.split(RE_MLSD_EQ);
    ret[kv[0].toLowerCase()] = kv[1];
  });
  ret.size = parseInt(ret.size);
  var modify = ret.modify;
  if (modify) {
